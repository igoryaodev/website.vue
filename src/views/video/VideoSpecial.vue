<template>
  <div class="exhibiton-wrap">
    <div class="hidden-xs">
    <div class="h33"></div>
    <div class="common-title">{{$t('activity.special')}}</div>
    <div class="h23"></div>
    ...
    <div class="visible-xs mvideo-wrap">
      <div class="mobile-common_title">{{$t('activity.special')}}</div>
  ...
  methods: {
    loadFilter() {
      if (this.selectTime !== this.$t('activity.all')) {
        this.$router.push({ name: 'video-special', query: { createTime: dateFns.format(this.selectTime, 'YYYY-MM-DD') } })
      } else {
        this.$router.push({ name: 'video-special' })
      }
      this.$router.go(0)
    },
    search(date) {
      if (date !== this.$t('activity.all')) {
        this.$router.push({ name: 'video-special', query: { createTime: dateFns.format(date, 'YYYY-MM-DD') } })
      } else {
        this.$router.push({ name: 'video-special' })
      }
      this.$router.go(0)
    ...
    load() {
      api.getvideoList({ page: 1, videoType: 3 }).then((data) => {
        this.videos = data
    ...
    loadMore() {
      this.loading = true
      this.show = true
      api.getvideoList({ page: this.page, videoType: 3, createTime: this.$route.query.createTime })
      .then((data) => {
      ...
      //   api.getvideoList({ page: this.page += 1, videoType: 3 }).then((data) => {
    ...
    timeLine() {
      api.getVideoDate({ videoType: 3 }).then((data) => {
        if (data) {
          this.timeLines = data
        }
