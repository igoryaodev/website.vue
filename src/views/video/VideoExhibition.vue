<template>
  <div class="exhibiton-wrap">
    <div class="hidden-xs">
    <div class="h33"></div>
    <div class="common-title">{{$t('activity.exhibition')}}</div>
    <div class="h23"></div>
    ...
    <div class="visible-xs mvideo-wrap">
      <div class="mobile-common_title">{{$t('activity.exhibition')}}</div>
  ...
  methods: {
    loadFilter() {
      if (this.selectTime !== this.$t('activity.all')) {
        this.$router.push({ name: 'video-exhibition', query: { createTime: dateFns.format(this.selectTime, 'YYYY-MM-DD') } })
      } else {
        this.$router.push({ name: 'video-exhibition' })
      }
      this.$router.go(0)
    },
    search(date) {
      if (date !== this.$t('activity.all')) {
        this.$router.push({ name: 'video-exhibition', query: { createTime: dateFns.format(date, 'YYYY-MM-DD') } })
      } else {
        this.$router.push({ name: 'video-exhibition' })
      }
      this.$router.go(0)
    ...
    load() {
      api.getvideoList({ page: 1, videoType: 1 }).then((data) => {
        this.videos = data
    ...
    loadMore() {
      this.loading = true
      this.show = true
      api.getvideoList({ page: this.page, videoType: 1, createTime: this.$route.query.createTime })
      .then((data) => {
      ...
      //   api.getvideoList({ page: this.page += 1, videoType: 1 }).then((data) => {
    ...
    timeLine() {
      api.getVideoDate({ videoType: 1 }).then((data) => {
        this.timeLines = data
