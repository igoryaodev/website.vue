<template>
  <div class="exhibiton-wrap">
    <div class="hidden-xs">
    <div class="h33"></div>
    <div class="common-title">{{$t('activity.pastExhib')}}</div>
    <div class="h23"></div>
    ...
    <div class="activity-list" v-if="!isActive">
      <div class="clearfix" v-infinite-scroll="loadMore" infinite-scroll-disabled="loading" infinite-scroll-distance="1">
        <exhibition-item v-for="activity in activityList.list" :key="activity.id" :activity="activity" :type="'3'"></exhibition-item>
      </div>
    ...
    <div class="activity-list" v-if="isActive">
      <div v-infinite-scroll="loadMore" infinite-scroll-disabled="loading" infinite-scroll-distance="1">
        <exhibition-list v-for="activity in activityList.list" :key="activity.id" :activity="activity" :type="'3'"></exhibition-list>
      </div>
      <p v-show="show" class="page-infinite-loading"></p>
    </div>
    ...
    <div class="visible-xs mobile-common_wrap">
      <div class="mobile-common_title">{{$t('activity.pastExhib')}}</div>
...
export default {
  ...
  created() {
    ...
    // api.getExhibition({ searchType: 3, page: 1 }).then((data) => {
    //   this.activityList = data
    // })
    api.getExhibitionDate({ searchType: 3 }).then((data) => {
      if (data) {
        this.dateList = data
      }
  ...
  methods: {
    loadFilter() {
      if (this.selectTime !== this.$t('activity.all')) {
        this.$router.push({ name: 'exhibition-past', query: { createTime: dateFns.format(this.selectTime, 'YYYY-MM-DD') } })
      } else {
        this.$router.push({ name: 'exhibition-past' })
      }
      this.$router.go(0)
    },
    changeStyle1() {
    ...
    search(date) {
      if (date !== this.$t('activity.all')) {
        this.$router.push({ name: 'exhibition-past', query: { createTime: dateFns.format(date, 'YYYY-MM-DD') } })
      } else {
        this.$router.push({ name: 'exhibition-past' })
      }
      this.$router.go(0)
    ...
    loadMore() {
      ...
      setTimeout(() => {
        api.getExhibition(
          { searchType: 3, page: this.page, createTime: this.$route.query.createTime })
        .then((data) => {
