<template>
  <div class="exhibiton-wrap">
    <div class="hidden-xs">
    <div class="h33"></div>
    <div class="common-title">{{$t('activity.upcomingExhib')}}</div>
    <div class="h23"></div>
    ...
    <div class="activity-list" v-if="!isActive">
      <div class="clearfix" v-infinite-scroll="loadMore" infinite-scroll-disabled="loading" infinite-scroll-distance="1">
        <exhibition-item v-for="activity in activityList.list" :key="activity.id" :activity="activity" :type="'2'"></exhibition-item>
      </div>
    ...
    <div class="activity-list" v-if="isActive">
      <div v-infinite-scroll="loadMore" infinite-scroll-disabled="loading" infinite-scroll-distance="1">
        <exhibition-list v-for="activity in activityList.list" :key="activity.id" :activity="activity" :type="'2'"></exhibition-list>
      </div>
      <p v-show="show" class="page-infinite-loading"></p>
    </div>
    ...
    <div class="visible-xs mobile-common_wrap">
      <div class="mobile-common_title">{{$t('activity.upcomingExhib')}}</div>
...
export default {
  ...
  created() {
    ...
    // api.getExhibition({ searchType: 1, page: 1 }).then((data) => {
    //   this.activityList = data
    // })
    api.getExhibitionDate({ searchType: 1 }).then((data) => {
      if (data) {
        this.dateList = data
      }
  ...
  methods: {
    loadFilter() {
      if (this.selectTime !== this.$t('activity.all')) {
        this.$router.push({ name: 'exhibition-upcoming', query: { createTime: dateFns.format(this.selectTime, 'YYYY-MM-DD') } })
      } else {
        this.$router.push({ name: 'exhibition-upcoming' })
      }
      this.$router.go(0)
    },
    changeStyle1() {
    ...
    search(date) {
      if (date !== this.$t('activity.all')) {
        this.$router.push({ name: 'exhibition-upcoming', query: { createTime: dateFns.format(date, 'YYYY-MM-DD') } })
      } else {
        this.$router.push({ name: 'exhibition-upcoming' })
      }
      this.$router.go(0)
    ...
    loadMore() {
      ...
      setTimeout(() => {
        api.getExhibition(
          { searchType: 1, page: this.page, createTime: this.$route.query.createTime })
        .then((data) => {
