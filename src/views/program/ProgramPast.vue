<template>
  <div class="exhibiton-wrap">
    <div class="hidden-xs">
    <div class="h33"></div>
    <div class="common-title">{{$t('program.past')}}</div>
    <div class="h23"></div>
    ...
    <div class="visible-xs mobile-common_wrap">
      <div class="mobile-common_title">{{$t('program.past')}}</div>
...
export default {
  ...
  created() {
    // api.getProgram({ page: 1, timeType: 2 }).then((data) => {
    //   this.activityList = data
    // })
    api.getProgramDate({ timeType: 2 }).then((data) => {
      if (data) {
        this.dateList = data
      }
  ...
  methods: {
    loadFilter() {
      if (this.selectTime !== this.$t('activity.all')) {
        this.$router.push({ name: 'program-past', query: { createTime: dateFns.format(this.selectTime, 'YYYY-MM-DD') } })
      } else {
        this.$router.push({ name: 'program-past' })
      }
      this.$router.go(0)
    },
    changeStyle1() {
    ...
    search(date) {
      if (date !== this.$t('activity.all')) {
        this.$router.push({ name: 'program-past', query: { createTime: dateFns.format(date, 'YYYY-MM-DD') } })
      } else {
        this.$router.push({ name: 'program-past' })
      }
      this.$router.go(0)
    ...
    loadMore() {
      ...
      setTimeout(() => {
        api.getProgram(
          { timeType: 2, page: this.page, createTime: this.$route.query.createTime })
        .then((data) => {
